...
from .models import Notification , Activity , Reply , Comment, Code , RechargeRequest, Profile, Post, LikePost, FollowersCount , Subject , GetPremium , News , Instructor , BuyLesson , Info , Chapter , ChapterLecture , BuyChapter
...
def inbox(request):
    ...
    messages = Reply.objects.filter(replyed_to=request.user.username).values().order_by('-created_at')
    ...
    if request.method == 'POST':
        ...
        if buy_filter == None:
            ...
            if user_profile.money < post.price:
               ...
               new_buy = BuyLesson.objects.create(post_id=post_id, username=username , name = user_profile.name , image=user_profile.image , method='wallet')
               new_buy.save()
               lesson_price = post.price
    ...
    if request.method == 'POST':
        ...
        if buy_filter == None:
            ...
            if user_profile.money < chapter.price:
               ...
               new_buy = BuyChapter.objects.create(chapter_id=chapter_id, username=username , name=user_profile.name , image=user_profile.image , method='wallet')
               new_buy.save()
               ...
               for x in chapter_parts:
                    if BuyLesson.objects.filter(post_id=x.lecture_id , username=request.user.username).first():
                        ...
                        buy_lecture = BuyLesson.objects.create(post_id=x.lecture_id , username=request.user.username , name=user_profile.name , image=user_profile.image , method='chapter')
